...
dnn_clf = joblib.load('core/models/dnn.pkl')
    ...
    if request.method == 'POST':
        ...
        if isBaby == 'NotBC_training':
          ...
          return Response({'prediction': isBaby}) 
            ...
            img = save_image_from_sound(temp_file_path, audio_file)

            # y = read_audio(temp_file_path, True)
            # spectrogram = audio_to_melspectrogram(y)

            img_tensor = torch.from_numpy(spectrogram)   # Assuming correct shape for your model 
         # ... Add any other required preprocessing ...
            ...
            os.remove(temp_file_path)
            print(predicted)
            return Response({'prediction': predicted.item()}) 
